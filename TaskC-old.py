...
class TwoStationSystem:
    ...
    def FirstProcess(self, reqId, category):
        ...
        if self.env.now - (arriveTime + CategoryParams[category]['SERVICETIME']) == waitTimesInterrupt + waitTime:
        ...
        #self.waitTimes[category].append(self.env.now - (arriveTime + CategoryParams[category]['SERVICETIME']))
        # if(reqId == 3):
        #     print(f"3 (1) queue wait + interrupt - {waitTimesInterrupt + waitTime}")
        #     print(f"3 (1) Total wait - {self.env.now - (arriveTime + CategoryParams[category]['SERVICETIME']) }")
